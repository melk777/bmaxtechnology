...
export default function Home() {
  ...
  const submitLead = (event: FormEvent<HTMLFormElement>) => {
    ...
    const message = `Olá, quero falar com a Bmax Technology.\n\nNome: ${data.get("nome")}\nTelefone: ${data.get("telefone")}\nCPF ou CNPJ: ${data.get("documento")}\nE-mail: ${data.get("email")}\nMensagem: ${data.get("mensagem")}`;
    window.open(`https://wa.me/554598437229?text=${encodeURIComponent(message)}`, "_blank", "noopener,noreferrer");
  };
  useEffect(() => {
  ...
  return <main>
    ...
    <section className="project" id="projeto"><p className="eyebrow">BMAX TECHNOLOGY</p><h2>Do projeto à instalação,<br />a sua energia começa aqui.</h2><p>Residências, empresas, hotéis, estacionamentos, eletropostos e revendedores no Sul do Brasil.</p><a className="button light" href={whatsapp} target="_blank">Chamar no WhatsApp <span>→</span></a></section>
    <section className="lead-form" id="contato"><div><p className="eyebrow ink">FALE COM A BMAX</p><h2>Encontre a solução ideal para a sua recarga.</h2><p>Preencha seus dados e receba um atendimento personalizado para seu projeto.</p></div><form onSubmit={submitLead}><label>Nome completo<input name="nome" type="text" autoComplete="name" required /></label><label>Telefone<input name="telefone" type="tel" autoComplete="tel" required /></label><label>CPF ou CNPJ<input name="documento" type="text" inputMode="numeric" required /></label><label>E-mail<input name="email" type="email" autoComplete="email" required /></label><label className="full">Mensagem<textarea name="mensagem" rows={5} required /></label><button className="button light" type="submit">Solicitar atendimento <span>→</span></button><p className="form-note">Ao enviar, você será direcionado ao WhatsApp da Bmax.</p></form></section>
    <footer><span>© 2026 Bmax Technology</span><span>Mobilidade elétrica para o Sul do Brasil.</span></footer>
  </main>;
}
